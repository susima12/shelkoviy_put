...
import { createToken, errorResponse, getUserFromRequest, jsonResponse, parseJsonBody, verifyPassword } from "@/server/auth";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const updatedUser = { ...user, email: mail };
        const token = await createToken(updatedUser);
        return jsonResponse({ ok: true, email: mail, user: updatedUser, token });
      },
    },
  },
});
